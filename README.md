...
> **Fork 版本**：本專案 fork 自 [`Mapleeeeeeeeeee/cc-session-reader`](https://github.com/Mapleeeeeeeeeee/cc-session-reader)，新增對 `CLAUDE_CONFIG_DIR` 環境變數的完整支援（CLI 與安裝腳本皆尊重，未設定時回退 `~/.claude`，詳見下方說明）。安裝與 release 來源為本 fork `beards/cc-session-reader`。
...
```bash
curl -fsSL https://raw.githubusercontent.com/beards/cc-session-reader/main/install.sh | bash
```
...
```powershell
irm https://raw.githubusercontent.com/beards/cc-session-reader/main/install.ps1 | iex
```
...
- **下載 Binary**：從 [GitHub Releases](https://github.com/beards/cc-session-reader/releases) 下載對應平台壓縮檔，解壓後放到 PATH。
- **僅安裝 Skill**：`mkdir -p "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cc-session" && curl -o "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cc-session/SKILL.md" https://raw.githubusercontent.com/beards/cc-session-reader/main/SKILL.md`

### Claude config 目錄（`CLAUDE_CONFIG_DIR`）

CLI 與安裝腳本都尊重 Claude Code 的 `CLAUDE_CONFIG_DIR` 環境變數。若有設定，session 記錄、Skill、usage 資料與 `config.json` 都會在 `$CLAUDE_CONFIG_DIR` 之下；未設定時回退到 `~/.claude`。本文件其餘出現的 `~/.claude` 路徑，在有設定時請改以該目錄為準。
...
```bash
mkdir -p "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cc-session"
curl -o "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cc-session/config.json" \
  https://raw.githubusercontent.com/beards/cc-session-reader/main/config.json.template
```
...
rm -rf "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/cc-session"
```

## Contributing

遇到 bug 或有功能需求，歡迎開 issue：
https://github.com/beards/cc-session-reader/issues
